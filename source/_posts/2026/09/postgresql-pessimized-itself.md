...
40和4w对比，pg选择了40。看起来pg选择了代价更小的执行方式，但是实际执行的时候却花费了比查询4w条数据更大的代价，这条sql的执行时间从0.2s上升到了17s。原因就是4w条`pending`状态的task并不是均匀分布的，为了找到`created_at`最小的`pending`数据，pg至少读了163w条数据，而不是推测的40多条。
